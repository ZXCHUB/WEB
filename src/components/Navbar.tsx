    ...
    <nav className="border-b border-red-900/30 bg-black/50 backdrop-blur-md sticky top-0 z-50">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 text-red-500 font-bold text-2xl tracking-tighter">
          ...
          <img src="https://api.dicebear.com/7.x/bottts/svg?seed=zxchub&backgroundColor=transparent&colors=red" alt="Logo" className="w-8 h-8 rounded-full" />
          ZXCHUB
        </Link>
                ...
                <div className="absolute right-0 mt-2 w-64 bg-[#0a0a0a] border border-zinc-800 rounded-xl shadow-2xl py-2 z-50">
                  <div className="px-4 py-3 border-b border-zinc-800 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Link to={`/profile/${user.uid}`} onClick={() => setIsMenuOpen(false)} className="text-xs text-zinc-500 hover:text-red-400 transition-colors">View your profile</Link>
                    </div>
                  </div>
