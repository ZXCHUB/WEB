...
import { ExternalLink, ShoppingCart, Globe, ShieldAlert, CheckCircle2, XCircle, AlertCircle, User } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {executors.map((executor, index) => (
            <motion.div
              ...
              className="p-6 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center gap-6 hover:border-red-900/30 transition-colors cursor-pointer"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link 
                  to={`/profile/${executor.authorId}`} 
                  onClick={(e) => e.stopPropagation()}
                  className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-red-400 transition-colors mb-2 w-fit"
                >
                ...
                <div className="flex gap-2 mt-2">
                  {executor.website && (
                    <a href={executor.website} target="_blank" rel="noreferrer"
                  ...
                  {executor.purchaseUrl && (
                    <a href={executor.purchaseUrl} target="_blank" rel="noreferrer"
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-600/20 hover:bg-red-600/30 text-red-400 text-xs font-medium transition-colors border border-red-900/40"
                      onClick={e => e.stopPropagation()}
                    >
