...
import { ShieldAlert, CheckCircle2, Clock, Calendar, Code2, Heart, Eye, Copy, Check, Ban } from 'lucide-react';
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (!usersSnapshot.empty) {
        ...
        // 2) Fallback: try as UID
        const directUserDoc = await getDoc(doc(db, 'users', value));
        if (directUserDoc.exists()) {
          foundUserDoc = directUserDoc;
        }
      }
      ...
      const scriptsQuery = query(
        ...
        where('authorId', '==', foundUserDoc.id),
        orderBy('createdAt', 'desc')
      );

      const scriptsSnapshot = await getDocs(scriptsQuery);
      setUserScripts(
        scriptsSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        }))
      );
          ...
          <div className="absolute -bottom-16 left-8 flex items-end gap-6">
            ...
            <div className="pb-4">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm">
                <RoleBadge role={profileUser.role} isOfficial={profileUser.isOfficial} />
                {isUserBanned(profileUser) && (
                  <span className="px-2.5 py-1 rounded-md bg-red-950/50 text-red-500 border border-red-900/50 font-bold flex items-center gap-1.5">
                ...
                {script.image ? (
                  <div className="h-40 overflow-hidden relative">
                    ...
                    <div className="absolute top-3 right-3 z-20 flex gap-2">
                      {script.isPremium && (
                        <span className="px-2 py-0.5 rounded-md bg-red-900/80 backdrop-blur-md text-white text-[10px] font-bold border border-red-500/50 shadow-[0_0_10px_rgba(220,38,38,0.5)]">
                          PREMIUM
                        </span>
                      )}
                      {script.executor && (
                        <span className="px-2 py-0.5 rounded-md bg-black/60 backdrop-blur-md text-red-400 text-[10px] font-bold border border-red-900/30">
                          {script.executor}
                        </span>
                      )}
                    </div>
